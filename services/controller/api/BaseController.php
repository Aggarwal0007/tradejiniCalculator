...
class BaseController
{
    ...
    protected function sendSuccess($data, $isArray = false)
    {
    
        $httpHeaders = array('Content-Type: application/json', 'HTTP/1.1 200 OK');
        if ($isArray) {
            $convertedData = json_encode($data[0]);
            $count = $data[1][0]['COUNT(*)'];
            $res = stripslashes(json_encode(array('status' => "ok", "d" => json_decode($convertedData), "count" => $count)));
        } else {
